...
install_homebrew() {
    ...
    # Add Homebrew to PATH if it's not automatically added
...
# Check if sed is installed (typically already installed on macOS)
if ! command -v sed &> /dev/null; then
    echo "sed not found. Installing sed..."
    brew install gnu-sed  # In case of issues with macOS BSD sed, install GNU sed
...
# Update .env with the provided values using macOS-compatible sed
